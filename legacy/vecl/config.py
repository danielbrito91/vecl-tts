...
# Define the specific character set for YourTTS fine-tuning
# This is taken directly from your provided script
...
class VeclArgs(VitsArgs):
    ...
    # Freeze the text encoder to preserve language understanding
    freeze_encoder: bool = True
    use_d_vector_file: bool = True

    # --- VECL-TTS specific parameters ---
    # Path to the pre-computed emotion embeddings for the dataset
    emotion_embedding_file: str = None

    # Dimension of the raw emotion embeddings (before projection)
    emotion_embedding_dim: int = 1024
    d_vector_dim: int = 512

    # Enable the emotion consistency loss
    use_emotion_consistency_loss: bool = True

    # Weight for the emotion consistency loss
...
class VeclConfig(VitsConfig):
    ...
    This class sets sensible defaults for fine-tuning YourTTS and
    incorporates the new VECL-TTS parameters.
    """

    # Override the model_args to use our custom VeclArgs
    model_args: VeclArgs = field(default_factory=VeclArgs)

    # Set the specific character set required by YourTTS
    characters: CharactersConfig = field(
        default_factory=lambda: YOURTTS_CHARACTERS
    )

    # --- Training Defaults for Fine-tuning ---
    epochs: int = 1000
    ...
    # Enable weighted sampling to balance languages in the batch
    use_weighted_sampler: bool = True
    weighted_sampler_attrs: dict = field(
        default_factory=lambda: {'language': 1.0}
    )

    # Dictionary to customize weights for specific attribute values.
    # Must be initialized to an empty dict (rather than None) to prevent
    # AttributeError inside Coqui-TTS `Vits.get_sampler`, which expects
    # `.get()` to be available.
    ...
    max_audio_len: int = (
        22050 * 20
    )  # Max audio length in samples (e.g., 20 seconds)

    def __post_init__(self):
        # Update num_chars based on the character set
        self.model_args.num_chars = len(self.characters)

        # Ensure the field is always a dictionary — some serialization paths
        # (e.g., when saving/loading via Coqpit) might convert empty dicts to
        # `None`. This prevents AttributeError later in `Vits.get_sampler()`.
